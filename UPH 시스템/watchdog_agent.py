...
def push_to_db(engine, rows, chunk_size=50):
    """대량 행을 psycopg2의 execute_values로 upsert.
    이러면 청크(기본 50행)당 네트워크 왕복이 '딱 1번'만 발생한다.

    ⚠️ 2026-09-01: chunk_size를 2000 -> 300으로 낮춤. UPH 검색 CS 필터를 "전체"로
    연 뒤(v1.3.17) 한 회차 변경분이 1,500~1,700건대로 커지면서, 그 전엔 1,000건대도
    순식간에 끝나던 단일 INSERT가 갑자기 140초 넘게 걸리다 statement timeout으로
    취소되는 게 3회 연속 재현됨(같은 배치가 캐시 미반영으로 계속 재시도되며 진행이
    전혀 안 됨). 원인이 단순 행 수 증가가 아니라 락 경합 등일 가능성이 있어 확실히
    규명은 못 했지만, 청크를 작게 쪼개면 (a) 각 문장이 타임아웃 안에 끝날 확률이
    높아지고 (b) 실패해도 잃는 작업량이 줄어든다.

    ⚠️ 2026-09-09: chunk_size를 300 -> 50으로 재차 낮춤. 매크로가 몇 시간 멈춰있다가
    재시작되면서 밀린 변경분이 12,000건대로 쌓였는데, 300행 청크의 '첫 청크'조차
    끝까지 못 가고 매번 timeout으로 취소돼 회차마다 진행이 전혀 안 되는 상황이
    실제로 발생함. 청크를 더 잘게 쪼개서 최소한 앞쪽 청크는 커밋되게 해 조금씩이라도
    전진하도록 함.
